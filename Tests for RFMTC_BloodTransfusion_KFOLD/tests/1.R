...
{
  checkEquals(
    "BLOOD TRANSFUSION RFMTC K-FOLD CROSS VALIDATION DATASET",
    RUN_TITLE,
...
{
  checkTrue(avgResDFLen == 748, "<avgResDFLen> length mismatch")
  checkTrue(length(custIDvector) == 748, "<custIDvector> length mismatch")
}
...
{
  checkTrue(hasCustID == FALSE, "<hasCustID> boolean mismatch")
}
...
{
  checkEqualsNumeric(custChurnColIndex, 5, "<custChurnColIndex> numeric mismatch")
}
...
{
  checkEqualsNumeric(REFERENCE_OPTIMALS, c(0.111, 3.72, 9.52), "<REFERENCE_OPTIMALS> wrong initiation")
...
test.findOptimumValues <- function()
{
  ...
  if (((RFMTC_PARAM_RANGE_LIST[1] %in% RFMTC_PARAM_RANGE_LIST_MIN[1]) 
      ...
      && (RFMTC_PARAM_RANGE_LIST[3] %in% RFMTC_PARAM_RANGE_LIST_MIN[3])) == TRUE)
  {
    expect_equal(b_RangeDetailsList[["MAX"]],31.09328, tolerance=1e-3)
    expect_equal(b_RangeDetailsList[["MIN"]],1.94333, tolerance=1e-3)
  }
  # check the output
  expect_that(
    findOptimumValues(
...
test.exl1_OptimalCalc <- function()
{
  ...
  # check the output refRfmtcReadyDF
   checkEqualsNumeric(length(refRfmtcReadyDF), 11, "<refRfmtcReadyDF> columns length mismatch")
  # refRfmtcReadyDF
  expect_that(
